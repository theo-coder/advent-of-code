...
fn solve_part2(input: &str) -> i64 {
    ...
    'outer: for operation in &operations {
        ...
        let nb_calc = 3_i64.pow((operation.1.len() - 1) as u32);

        for i in 0..nb_calc {
            let mut calc_tpl = String::new();
            let mut nb = i;

            while calc_tpl.len() < (operation.1.len() - 1) {
                calc_tpl.insert(0, char::from_digit((nb % 3) as u32, 3).unwrap());
                nb /= 3;
            }
...
fn calc_from_binary(nbs: &[i64], binary_ops: &str) -> i64 {
    ...
    for (nb_idx, nb) in nbs.iter().enumerate().skip(1) {
        match binary_ops.chars().nth(nb_idx - 1).unwrap() {
            '0' => result += nb,
            '1' => result *= nb,
            '2' => result = format!("{}{}", result, nb).parse::<i64>().unwrap(),
            _ => unreachable!(),
        }
    }
